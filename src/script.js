...
const visual_manager = (() => {
    ...
    const playerOne_selector = document.getElementById("playerOne_selector");
    const playerTwo_selector = document.getElementById("playerTwo_selector");
    ...
    newGame_button.addEventListener('click', () => {
        ...
        //     dropdown.addEventListener("change",newGame);
        // });
    });
...
const game_manager = (() => {
    ...
    function player_move(player, currentS, move) {
        do {
            [x, y] = player.set_move(move);
        } while (!check_playable(x, y));
        game_board.set_box(x, y, currentS);
        round++;
    ...
    function minimax(depth,difficulty) {
        ...
        if (ende) return { value: ende };       //win situation
        if (ende == 0) return { value: ende };    //draw situation
        ...
        for (let move of game_board.get_validMoves()) {
            ...
            if (value >= best.value) {
                if (value != 10 && value < 10) {
                    ...
                    best.dummy_moves.push(move); // keep track of equally valued moves
                }
            }
        }
        return best;
